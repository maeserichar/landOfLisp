...
(defun objects-at (loc objs objs-locs)
  (labels ((at-loc-p (obj)
		     (eq (cadr (assoc obj objs-locs)) loc)))
    (remove-if-not #'at-loc-p objs)))

(defun describe-objects (loc objs objs-locs)
  (labels ((describe-obj (obj)
			 `(you see a ,obj on the floor.)))
    (apply #'append (mapcar #'describe-obj (objects-at loc objs objs-locs)))))
...
(defun tweak-text (lst cap lit)
  (when lst
    (let ((item (car lst))
	  (rest (cdr lst)))
      (cond ((eq item #\space) (cons item (tweak-text rest cap lit)))
	    ((member item '(#\! #\? #\.)) (cons item (tweak-text rest t lit)))
	    ((eq item #\") (tweak-text rest caps (not lit)))
	    (lit (cons item (tweak-text nil lit)))
	    ((or cap lit) (cons (char-upcase item) (tweak-text rest nil lit)))
